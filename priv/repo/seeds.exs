...
deck_names = ["Yolo Deck", "Hardest Verbs"]

Enum.each(deck_names, fn name ->
  {:ok, deck} = JapaneseVerbConjugation.Decks.create_deck(%{name: name})

  Enum.each(1..15, fn _ ->
    card = JapaneseVerbConjugation.VerbTenses.list_verb_tenses() |> Enum.random()

    {:ok, _} =
      JapaneseVerbConjugation.CardDecks.create_card_deck(%{deck_id: deck.id, card_id: card.id})
  end)
end)
